...
namespace com
{
	...
	string Md5::Encrypt(const string& src, bool bCapital /*= true*/)
	{
		string result = "";

		result = MD5String(src.c_str());
		if (bCapital)
		{
			result = StringTool::ToUpper(result);
		}
...
const char* MD5String(const char *string)
{
	...
	for (i = 0; i < 16; i++)
	{
		sprintf_s(&(output1[2 * i]), 3, "%02x", (unsigned char)digest[i]);
		sprintf_s(&(output1[2 * i + 1]), 3, "%02x", (unsigned char)(digest[i] << 4));
...
*/
void StringAddOne(char * orstring)
{
	unsigned int len;
	int i, n;
